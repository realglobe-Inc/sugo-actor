...
class SugoActor extends SugoClient {
  ...
  async connect () {
    ...
    async function doConnect () {
      ...
      await asleep(0)
      const socket = s.socket = sgSocketClient(url, {multiplex, reconnection, path})
      socket.on(ERROR, (err) => {
      ...
      for (let name of Object.keys(modules || {})) {
        await s.load(name, modules[name])
      }

      socket.on(JOIN, (data) => {
        const {caller, messages} = data || {}
        debug('Caller joined', caller)
        s.emitLocally(CallerEvents.JOIN, {
      ...
      socket.on(LEAVE, (data) => {
        ...
        s.emitLocally(CallerEvents.LEAVE, {
          caller: s.asCallerInstance(caller),
          messages
        })
      })
      socket.on(NOTICE, ({name, data}) => {
        switch (name) {
          case CALLER_GONE: {
            const caller = s.asCallerInstance({key: data.key})
            debug('Caller gone', caller)
            s.emitLocally(CallerEvents.LEAVE, {caller})
            break
          }
          default: {
            break
    ...
    // Load sub modules
    {
      for (let key of Object.keys(module)) {
        ...
        if (Module.isModule(val)) {
          ...
          if (!hasEmitter) {
            await s.loadSub(moduleName, {[key]: val})
          }
        }
      }
    }
  ...
  async loadSub (moduleName, subModules) {
    ...
    for (let subModuleName of Object.keys(subModules)) {
      ...
      await s.load(fullName, subModules[subModuleName])
    }
  }
  ...
  async unloadSub (moduleName, subModuleNames) {
    ...
    for (let subModuleName of subModuleNames) {
      await s.unload([moduleName, subModuleName].join('.'))
    }
  }
  ...
  asCallerInstance (caller) {
    ...
    const emit = (event, data) => s.emitToRemote(event, data, {only})
