...
const { JOIN, LEAVE, SPEC, DESPEC, PERFORM } = RemoteEvents
...
class SugoActor extends SugoClient {
  ...
  connect () {
    ...
    let doConnect = () => co(function * () {
      ...
      socket.on(JOIN, (data) => s.emit(CallerEvents.JOIN, data))
      socket.on(LEAVE, (data) => s.emit(CallerEvents.LEAVE, data))
  ...
  unload (moduleName) {
    ...
    return co(function * () {
      ...
      yield socket.call(DESPEC, { name: moduleName })
      let pipe = s.pipes[ moduleName ]
      module.$$unregisterEmitter(pipe)
      delete module.$$actor
      pipe.close()
      delete s.pipes[ moduleName ]
