...
const SpotValidator = require('./helpers/spot_validator')
...
class SugoSpot extends EventEmitter {
  ...
  connect () {
    const s = this
    let { validator, key, token, force, interfaces } = s
    return co(function * () {
      if (s.socket) {
      ...
      socket.on(PERFORM, (data, callback) => co(function * () {
        let errors = validator.validatePerformConfig(data)
        if (errors) {
          callback(ng(new Error(`Invalid config: ${errors[ 0 ].message}`)))
        }
        ...
        pipe.open()
        try {
          let result = yield interface_.$$invoke(method, params, pipe)
          pipe.close()
          callback(ok(result))
        } catch (err) {
          pipe.close()
